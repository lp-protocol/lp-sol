...
contract TheLPTest is Test {
  ...
  function setUp() public {
    ...
    lp = new TheLP(block.timestamp, renderer);
  ...
  function testMint_should_revert_if_not_started() public {
    lp = new TheLP(block.timestamp + 10 days, renderer);
  ...
  function testMint_should_start_at_tokenID_1() public {
    uint256 price = lp.getCurrentMintPrice();
    lp.mint{ value: price }(1);
    address owner = lp.ownerOf(1);
  ...
  function testMint_should_refund_additional_amount() public {
    ...
    assertEq(balanceBeforeMint - balanceAfterMint, 3.33 ether);
  }
  ...
  function testMint_should_lock_it_in_and_transfer_funds() public {
    for (uint256 i = 0; i < 40; i++) {
      if (i == 39) {
        ...
        emit PaymentReceived(address(lp), (3.33 ether * 10000) / 2);
      }
      lp.mint{ value: 3.33 ether * 250 }(250);
    }
